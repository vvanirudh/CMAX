'''
Discrete MPC controller for the FetchPush Env
'''
import numpy as np
import heapq as hq
...
class MPCController(Controller):
    ...
    def act(self, obs):
        ...
        for _ in range(self.num_expansions):
            obs, actions_from_root = hq.heappop(queue)[-1]
            ...
            for action in self.actions:
                ...
                mj_sim_state.qpos[:] = obs['sim_state'].qpos[:]
                mj_sim_state.qvel[:] = obs['sim_state'].qvel[:]
                self.model.env.sim.set_state(mj_sim_state)
                self.model.env.sim.forward()

                child_obs, _, _, _ = self.model.step(
                    self.action_scale * np.array(action))
                mj_sim_state = self.model.env.sim.get_state()
                child_obs['sim_state'] = mj_sim_state

                cost = self.heuristic(child_obs, child_actions_from_root)

                node = cost + ((child_obs,
                                child_actions_from_root),)
        ...
        return np.array(best_actions[0])
    ...
    def heuristic(self, obs, actions_from_root):
        ...
        target_gripper_position[0] += -1. * \
            np.sin(block_to_goal_angle) * block_width / 2.
        target_gripper_position[1] += -1. * \
            np.cos(block_to_goal_angle) * block_width / 2.
        target_gripper_position[2] += 0.005

        object_to_goal_distance = int(
            10000 * np.linalg.norm(np.subtract(block_position[:2], goal[:2])))
        gripper_to_target_distance = len(actions_from_root) + int(
            10000 * np.linalg.norm(np.subtract(gripper_position, target_gripper_position)))
