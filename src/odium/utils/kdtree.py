'''
Blatantly copied from scipy kdtree implementation for the purposes of understanding the code
'''
...
class KDTree(object):
    ...
    class innernode(node):
        def __init__(self, split_dim, split, less, greater, size=None):
    ...
    def __build(self, idx, maxes, mins):
        if len(idx) <= self.leafsize:
            ...
            size = np.max(dimensions)
            return KDTree.leafnode(idx, size=size)
        else:
            data = self.data[idx]
            ...
            return KDTree.innernode(d, split,
                                    ...
                                    self.__build(idx[greater_idx], maxes, greatermins), size=size)
    ...
    def query_knownness(self, x, p=2, alt=False):
        '''
        Query the kd-tree for the knownness of points in x
        knownness is defined in the paper

        Nouri, Ali, and Michael L. Littman. "Multi-resolution exploration in continuous spaces." Advances in neural information processing systems. 2009.

        Created by Anirudh Vemula
        '''
        x = np.asarray(x)
        if np.shape(x)[-1] != self.m:
            raise ValueError("x must consist of vectors of length %d but has shape %s" % (
        ...
        if retshape != ():
            # More than one point
            knownnesses = np.empty(retshape, dtype=np.float)
            for c in np.ndindex(retshape):
                # For each point
                knownness = self.__query_knownness(x[c], p, alt)
                knownnesses[c] = knownness

            return knownnesses
        else:
            # Only one point
            knownness = self.__query_knownness(x, p, alt)
            return knownness

    def __query_knownness(self, x, p=2, alt=False):
        side_distances = np.maximum(
            0, np.maximum(x - self.maxes, self.mins - x))
        if p != np.inf:
            side_distances **= p
            min_distance = np.sum(side_distances)
        else:
        ...
        while q:
            min_distance, side_distances, node = heappop(q)
            if isinstance(node, KDTree.leafnode):
                # Leaf node, compute knownness
                # This is the leaf node corresponding to the region containing the given point x
                # Number of points in this leaf node
                count = node.children
                # Maximum number of points it can contain
                nu = self.leafsize
                # inf-norm of the size of the region it covers
                size = node.size
                # Normalizing size of the tree
                mu = (self.tree.size) / \
                    np.floor((self.n * self.m / (1.0 * nu)) ** (1.0/self.m))
                # Compute knownness
                knownness = min(1, ((1.0 * count) / nu) * ((1.0 * mu) / size))
                # Alternative knownness from
                # Hofer, L., & Gimbert, H. (2016). Online Reinforcement Learning for Real-Time Exploration in Continuous State and Action Markov Decision Processes, (Behnke 2006). Retrieved from http://arxiv.org/abs/1612.03780
                alternative_knownness = min(
                    1, ((1.0 * count / size) / (1.0 * self.n / self.tree.size)))
                if not alt:
                    return knownness
                else:
                    return alternative_knownness
            else:
                # compute minimum distances to the children and push them on
                if x[node.split_dim] < node.split:
                    near, far = node.less, node.greater
