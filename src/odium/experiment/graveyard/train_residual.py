...
from odium.agents.her_residual_agent.arguments import get_args
from odium.agents.her_residual_agent.her_residual_agent import her_residual_agent
...
def launch(args):
    ...
    if MPI.COMM_WORLD.Get_rank() == 0 and args.log_dir:
        logger.configure(dir=os.path.join(
            'logs', 'residual_her', args.log_dir), format_strs=['tensorboard', 'log', 'csv', 'json', 'stdout'])
    ...
    her_trainer = her_residual_agent(args, env, env_params)
    her_trainer.learn()
