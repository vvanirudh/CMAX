...
class Geometry(URDFType):
    ...
    def copy(self, prefix='', scale=None):
        ...
        v = Geometry(
            ...
            cylinder=(self.cylinder.copy(prefix=prefix, scale=scale)
                      if self.cylinder else None),
            sphere=(self.sphere.copy(prefix=prefix, scale=scale)
                    if self.sphere else None),
            mesh=(self.mesh.copy(prefix=prefix, scale=scale)
...
class Transmission(URDFType):
    ...
    def __init__(self, name, trans_type=None, joints=None, actuators=None):
        self.name = name
    ...
    def _from_xml(cls, node, path):
        kwargs = cls._parse(node, path)
        kwargs['trans_type'] = 'SimpleTransmission'
...
class Link(URDFType):
    ...
    def copy(self, prefix='', scale=None):
        ...
        cpy = Link(
            ...
            collisions=[v.copy(prefix=prefix, scale=scale)
    ...
    def link_fk_batch(self, cfgs=None, link=None, links=None, use_names=False):
        ...
        for lnk in self._reverse_topo:
            ...
            for i in range(len(path) - 1):
                ...
                poses = np.matmul(joint.get_child_poses(
                    cfg_vals, n_cfgs), poses)
    ...
    def animate(self, cfg_trajectory=None, loop_time=3.0, use_collision=False):
        ...
        # If it is specified, parse it and extract the trajectory length.
            ...
            if ct.ndim != 2 or ct.shape[1] != len(self.actuated_joints):
                raise ValueError(
                    'Cfg trajectory must have entry for each joint')
        ...
        if isinstance(cfgs, dict):
            ...
            if isinstance(cfgs[0], dict):
                for cfg in cfgs:
                    for joint in cfg:
                        if isinstance(joint, six.string_types):
                            joint_cfg[self._joint_map[joint]].append(
                                cfg[joint])
                        else:
